 ...
 *   node bazantic-recipes/demo-agent.mjs [--capability "OCR"] [--execute]
 *
 * Flags:
 *   --capability "X"   Search for a capability (default: from goal keyword in recipe)
 *   --execute          Actually confirm payment (default: dry-run showing plan)
 *   --invoke           Also invoke the service after settlement
 */
...
const recipe = JSON.parse(readFileSync(resolve(__dirname, 'buy-service.json'), 'utf-8'));
...
for (const p of providers.slice(0, 5)) {
  ...
  if (p.reasoning) for (const line of p.reasoning) console.log(`     ${line}`);
}
